...
对类的使用主要是通过 import 导入类，new 实例化类。import 是别名操作，只在编译期起作用，运行期的时候会转换为 `Test test = new package.Test();` 执行。JVM 碰到 new 指令会判断元空间是否存在 class 类，如果不存在会执行前面的加载、链接和初始化操作。如果存在则直接使用，这就是 JVM 为了性能内存，执行的懒加载过程。
